...
function MoviesList(){
        const { genre, search, year, featured, category }= useParams()
    ...
    const form = useForm({
        ...
        API.get(`/movies/?skip=${skip}&take=${take}&search=${filters.search}&year=${filters.year}&featured=${filters.featured}&genre=${filters.genre}&category=${filters.category}`).then((response)=>{
    ...
    return(
        <Section Id='menu-list' AddPading={true}>
            ...
            {/* End Inputs Section*/}
            <div className="p-4 bg-blue-200 flex flex-col items-left justify-center gap-3 rounded-3xl">
            <p className="text-2xl">{category? (
                category=='featured'? 'Featured by Administrators': category=='views'? 'By User Views': category=='posted'? 'Latest Entries': category=='released'? 'Latest Releases': category=='califications'? 'By Best Ratings': category
            ): 'Movies'}</p>
            {category && (
                <p className="text-sm">{
                category=='featured'? 'Here you will find Movies Featured by Administrators.': category=='views'? 'Here you will find Movies Ordered By User Views.': category=='posted'? 'Here you will find our Latest Movies Entries.': category=='released'? 'Here you will find Movies Ordered by Most Recent Releases.': category=='califications'? 'Here you will find Movies Ordered By Rating.': category
            }</p>
            )}
